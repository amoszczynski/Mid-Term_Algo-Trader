...
def buyConfirmation(ticker, cross_day):
    ...
    for val in reversed(df['RSI'].tolist()):
        if val > 50 and val <80 and days < cross_day:
            rsi = True
            break
        else:
            days += 1
...
def sellSignal(ticker, time_frame):
    #here I want to use stoch and price falling under MA
    stoch = False
    p_under = False
    ...
    if stoch and p_under and (rsi or macd):
        return True
    else:
        return False

#print(buyConfirmation('NI', 4))
...
for position in portfolio:
    if sellSignal(position.symbol, 3) == True:
        api.close_position(position.symbol)
        #print(position.symbol)
    elif float(position.unrealized_plpc) <= -.5:
        api.close_position(position.symbol)
        #print(position.symbol)
    ...
    for t in buys:
        ...
        try:
            if shares != 0 and check == True:
               api.submit_order(t, shares, 'buy', 'limit', 'day', limit_price= (tprice * .975))
        except:
            if int(shares) != 0 and check == True:
               api.submit_order(t, int(shares), 'buy', 'limit', 'day', limit_price= (tprice * .975))
